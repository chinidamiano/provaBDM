...
plt.plot((t[:39]),v_df["abscoef"].values[:39],color="black",linestyle="-",linewidth="2",label="SCORE")
plt.plot((t[39:]),v_df["abscoef"].values[39:],color="black",alpha=0.35,linestyle="-",linewidth="2")
...
plt.savefig("./output/2-First_part/figures/Figure1d.pdf")

signature.to_csv("./output/2-First_part/theirM_signature.csv")
signature.to_pickle("./output/2-First_part/theirM_signature.p")

np.savetxt("./output/2-First_part/theirM_background_genes.txt",v_df.index.get_level_values(0).values,fmt="%s")
